...
>
	...
	>
		<span
			class={`h-0.5 w-7 bg-white transition-all duration-300 ${isMenuOpen ? "rotate-45 translate-y-2" : ""}`}
		></span>
		<span class={`h-0.5 w-7 bg-white transition-all duration-300 ${isMenuOpen ? "opacity-0" : ""}`}
		></span>
		<span
			class={`h-0.5 w-7 bg-white transition-all duration-300 ${isMenuOpen ? "-rotate-45 -translate-y-2" : ""}`}
		></span>
	...
	>
		<nav class="flex flex-col items-center gap-8">
			{#each tabs as tab, i}
				<a
					href={tab.link}
					class="font-bigshoulders text-3xl text-white hover:text-gold tracking-[0.2em] transition-colors"
		...
		<div class="absolute bottom-12 text-zinc-500 font-quicksand text-sm tracking-widest">
			27FOUR GUITARS
		</div>
	</div>
{/if}
